...
%%%===================================================================
-export([start_rand_stream/1,next_rand/1,rand_stream/1,cipher_stream/1,
...
decipher(Message,Unshift_amount)->
	lists:map(fun (Letter) -> Letter - Unshift_amount end, Message).
...
cipher_stream({Shift_amount,Messages})->
	receive
        {put, Message_sent} ->
            Encrypted_message = lists:map(fun (Letter) -> Letter + Shift_amount end, Message_sent),
            cipher_stream({Shift_amount, Messages ++ [Encrypted_message]});
        {request, Pid} -> 
            [First|Rest] = Messages,
            Pid ! {First},
            cipher_stream({Shift_amount,Rest})
    end.
